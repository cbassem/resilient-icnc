...
#ifndef MULTITYPECHECKPOINTMANAGER_H_
#define MULTITYPECHECKPOINTMANAGER_H_
...
class MultiTypeCheckpointManager: public CheckpointManager<Tag, Item> {

public:
	MultiTypeCheckpointManager(int step_collections, int tag_collections, int item_collections);
...
template<class Tag, class Item>
MultiTypeCheckpointManager<Tag, Item>::MultiTypeCheckpointManager(int step_collections, int tag_collections, int item_collections):
...
template<class Tag, class Item>
void MultiTypeCheckpointManager<Tag, Item>::processItemPut(Tag producer, int stepProducerColId, Tag key, Item item, int itemColId) {
    mutex_t::scoped_lock _l( m_mutex );
...
template<class Tag, class Item>
void MultiTypeCheckpointManager<Tag, Item>::processStepDone(Tag step, int stepColId, int puts, int prescribes) {
    mutex_t::scoped_lock _l( m_mutex );
...
template<class Tag, class Item>
void MultiTypeCheckpointManager<Tag, Item>::printCheckpoint() {
	mutex_t::scoped_lock _l( m_mutex );
...
template<class Tag, class Item>
std::tr1::unordered_map<Tag, Item >& MultiTypeCheckpointManager<Tag, Item>::getItemCheckpoint( int item_col_id ) {
	return item_checkpoint_[item_col_id];

}

template<class Tag, class Item>
std::set< Tag >& MultiTypeCheckpointManager<Tag, Item>::getTagCheckpoint( int tag_col_id ) {
	return tag_checkpoint_[tag_col_id];
}

template<class Tag, class Item>
void MultiTypeCheckpointManager<Tag, Item>::calculateCheckpoint() {
	mutex_t::scoped_lock _l( m_mutex );
	int crrCol(0);
...
template<class Tag, class Item>
void MultiTypeCheckpointManager<Tag, Item>::removeTagFromCheckpoint(Tag tag, int collection) {
	tag_checkpoint_[collection].erase(tag);
}

template<class Tag, class Item>
TagLog<Tag, Item>& MultiTypeCheckpointManager<Tag, Item>::getTagLog(int colid, Tag tag) {
...
#endif /* MULTITYPECHECKPOINTMANAGER_H_ */
